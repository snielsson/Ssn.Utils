...
namespace Ssn.Utils.Extensions {
    public static class StringExtensions {
        ...
        private static Regex GetCachedRegex(string pattern, RegexOptions regexOptions) {
            string key = pattern + (int) regexOptions;
        ...
        public static IEnumerable<string> ExtractSingleGroup(this string @this, string pattern, int group = 1, RegexOptions? options = null) {
            MatchCollection matches = GetCachedRegex(pattern, options ?? DefaultRegexOptions).Matches(@this);
        ...
        public static bool Matches(this string @this, string pattern, RegexOptions? options = null) {
            Regex regex = GetCachedRegex(pattern, options ?? DefaultRegexOptions);
        ...
        public static string Diff(this string @this, string str) {
            ...
            string[] lines1 = @this.Split(Environment.NewLine);
            string[] lines2 = str.Split(Environment.NewLine);
            for (int i = 0; i < Math.Min(lines1.Length, lines2.Length); i++) {
                string l1 = lines1[i];
                string l2 = lines2[i];
                if (l1 != l2) {
                    int limit = Math.Min(l1.Length, l2.Length);
        ...
        public static string ToSlug(this string @this, bool toLower = false) {
            if (@this == null)
                return "";
            string normalised = @this.Normalize(NormalizationForm.FormKD);
            const int maxlen = 80;
        ...
        public static string ToMD5Hash(this string @this) {
            lock (_md5) {
                byte[] bytes = _md5.ComputeHash(Encoding.UTF8.GetBytes(@this));
